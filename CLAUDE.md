...
- `sw.js`: cache name `ponte-v51`; install uses `fetch(url, { cache: 'reload' })` per file to bypass browser HTTP cache; network-first for `/api/*`; cache-first for everything else; old cache versions deleted on activate
...
- Update `CACHE_NAME` in `sw.js` (e.g. `ponte-v51`) after major frontend changes to bust service worker cache
