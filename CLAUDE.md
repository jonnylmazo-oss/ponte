...
- Response fields: `{italian, english, spanish, note, category, tense, root, pronunciation, wordType, baseForm, baseFormEN, example, exampleEN}`
  ...
  - `example` / `exampleEN`: natural Italian sentence using the word in context (10–15 words) + English translation; stored on flashcard when saved from tooltip
...
- `sw.js`: cache name `ponte-v55`; install uses `fetch(url, { cache: 'reload' })` per file to bypass browser HTTP cache; network-first for `/api/*`; cache-first for everything else; old cache versions deleted on activate
...
- Update `CACHE_NAME` in `sw.js` (e.g. `ponte-v55`) after major frontend changes to bust service worker cache
