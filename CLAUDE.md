...
dictionary.js      Translate tab IIFE — lookup + usage checker + deep-dive
                   expand-in-place (absorbed deep-dive.js, 2026-08-16)
...
Script load order: `utils.js` → `data/*.js` → `app.js` → `false-friends.js` → `grammar.js` → `flashcards.js` → `practice.js` → `dictionary.js` → `conversation.js` → `mission.js` → `progress.js` → `visual-cards.js` → `shadowing.js` → `audio-player.js` → `onboarding.js` (`visual-cards.js` must load after `flashcards.js` — it uses `window.ponteApplySmTwo` + `window.ponteSpeakCard`)
...
- **Translate** — bidirectional IT↔EN lookup + Usage Checker (AI grammar feedback) + the deep-dive expander above; lookup history chips (`ponte_dict_history`) now cover deep dives too
...
- **Deep dive** — no longer a separate tab: consolidated into **Translate** as a lazy expand-in-place section ("🔍 All meanings, examples & etymology") on the result card — senses → per-sense examples → etymology, fetched once per word per session, cached. `window.ponteDeepDive(word)` (called from both drills' flip-card backs) switches to Translate, looks the word up, and auto-expands. The card's single Save ★ is enriched with the primary sense's example when depth has loaded
